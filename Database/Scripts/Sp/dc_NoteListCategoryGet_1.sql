USE [MarkdownNotes]

IF object_id( N'dbo.dc_NoteListCategoryGet_1') IS NOT NULL
BEGIN 
 PRINT 'Dropping Procedure dbo.dc_NoteListCategoryGet_1...'
 DROP PROCEDURE dbo.dc_NoteListCategoryGet_1
END

PRINT 'Creating Procedure dbo.NoteListCategoryGet_1...'
SET QUOTED_IDENTIFIER ON
SET ANSI_NULLS ON
GO


CREATE PROCEDURE dbo.dc_NoteListCategoryGet_1
  @UserName nvarchar(Max),
  @ReturnValue nvarchar(50) output

AS
BEGIN
  SET NOCOUNT ON;
  SET XACT_ABORT ON;
  

  SELECT DISTINCT  ct.ID, ct.Name FROM dbo.Category ct 
  JOIN dbo.Users us ON ct.OwnerID = us.UserID 
  where us.Name = @UserName
  

/*GRANT EXEC ON dbo.dc_NotelistGet_1 TO sa 
SELECT [Password] FROM MarkdownNotes.dbo.Notes Where NoteName =  'User1'
*/
END
GO


   